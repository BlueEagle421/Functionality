...
import com.blueeagle421.functionality.item.custom.openable.InfernalSackItem;
...
public class OnLivingDrops {
    ...
    public static void onLivingDrops(LivingDropsEvent event) {
        ...
        if (!config().features.infernalSacks.limitEnabled.get())
            return;

        // read or create data
        InfernalSackChunkData data = server.getDataStorage()
                .computeIfAbsent(InfernalSackChunkData::load, InfernalSackChunkData::new,
                        InfernalSackChunkData.storageKey());

        for (ItemEntity itemEntity : event.getDrops()) {
            if (itemEntity.getItem().getItem() instanceof InfernalSackItem) {

                long chunkLong = getChunk(itemEntity.blockPosition());
                data.getCurrentForChunkOrInit(chunkLong);

                // do we have more sacks here?
                boolean allowed = config().items.infernalSack.enabled.get() && data.consumeOne(chunkLong);
